...
- [x] Rules of Hooks; linter enforcement and why
- [x] `useState` pitfalls: stale closures, functional updates, batching
- [x] `useReducer` for complex state; reducer purity, action typing
- [x] `useEffect` dependency arrays; synchronization vs lifecycle mental model
- [x] `useLayoutEffect` vs `useEffect`; layout thrash considerations
- [x] `useRef` for instance values, DOM refs, avoiding re-renders
- [x] `useMemo` and `useCallback`: when they help and when they hurt
- [x] `useImperativeHandle` with `forwardRef` for escape hatches
- [x] Concurrent features: `useTransition`, `useDeferredValue`
- [x] Custom hooks: composition, parameters, return structure, naming, reuse
- [x] Common gotchas: exhaustive-deps, subscribing/unsubscribing, event handlers
