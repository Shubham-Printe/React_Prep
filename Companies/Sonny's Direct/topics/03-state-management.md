...
1. What are the core pieces of Redux (store, actions, reducers), and how does RTK simplify them?
   - Store holds state, actions describe events, reducers compute next state. RTK simplifies setup with
     `configureStore`, `createSlice` (auto action creators + reducers), and sensible middleware defaults.
     Example:
     ```js
