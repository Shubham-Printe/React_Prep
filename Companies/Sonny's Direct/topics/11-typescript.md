...
1. What’s the difference between `type` and `interface` in TypeScript?
   - Both are ways to describe the shape of data. Think of them as “labels” for what fields an object must have.
     `interface` can be reopened and extended later (declaration merging), so it’s common for public APIs.
     `type` is more flexible because it can also describe unions, intersections, tuples, and primitives.
     Example:
     ```ts
...
2. What is a generic, and when would you use one?
   - A generic is a type “placeholder” so one function/type can work with many data types safely. It lets
...
3. When should you use a union vs a generic?
   - Use a union for a fixed set of options (`"success" | "error"`). Use a generic when you want the
     same structure to work for many types while preserving the actual type.
