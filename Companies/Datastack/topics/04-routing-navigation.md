...
1. What’s the difference between client-side routing and server-side routing, and what breaks on refresh/deep link if the server isn’t configured correctly?
   ...
   - **Fix**: serve `index.html` for app routes and real files for static assets (e.g. `/assets/*`).
     The client-side router then loads and shows the right page.

   - **Exact setting (by environment):**
     - **Nginx:** `try_files $uri $uri/ /index.html;` (try file, then directory, else `index.html`).
     - **Vercel:** In `vercel.json`: `"rewrites": [{ "source": "/(.*)", "destination": "/index.html" }]`.
     - **Netlify:** In `_redirects` or `netlify.toml`: `/* /index.html 200`.
     - **Vite dev:** Already does this by default; for production you configure the host (e.g. Nginx) the same way.
