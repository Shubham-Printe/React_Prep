...
6. [Understanding CD: what we're doing and why](#6-understanding-cd-what-were-doing-and-why)
6b. [Optional: deployment options](#6b-optional-deployment-options-how-to-add-cd)
...
## 6. Understanding CD: what we're doing and why

### What is CD here?

**CD = Continuous Deployment.** In this guide it means: *automatically putting your built app on a live URL whenever CI passes on `main`.*

- **CI** answers: “Does the code lint, test, and build?” (all on GitHub’s servers.)
- **CD** answers: “Is that built output actually running somewhere users can open?” (e.g. `https://your-app.vercel.app` or GitHub Pages.)

So when we add “deployment” to the pipeline, we’re adding a step (or a separate system) that takes the **output of `npm run build`** (usually the `dist/` folder) and **uploads it to a host** that serves it over HTTPS. No manual “build then drag files to a server” — push to `main`, and a few minutes later the site is updated.

### Why do it?

| Reason | Explanation |
|--------|-------------|
| **No manual deploy** | Push to `main` → CI runs → if green, the new build is deployed. You don’t FTP, run deploy scripts by hand, or “remember to deploy.” |
| **“Main is always live”** | The default branch reflects what’s actually running. Good for demos, portfolios, and small teams. |
| **Same artifact** | The same `dist/` that passed tests and build is what gets deployed — no “works on my machine” deploy. |
| **Fast feedback** | You see the real site with your changes shortly after merging. |

So in the guide, the **CD part** is: *once CI is stable, we add a way to take the built app and put it on a host (GitHub Pages, Vercel, or Netlify) so that “green main” automatically becomes the live site.*

### How it fits with the three options

- **GitHub Pages** — You add a **deploy job in the same GitHub Actions workflow**. That job runs only on push to `main`, uses the built `dist/` as an artifact, and publishes it to Pages. So: one workflow = CI + CD; everything stays inside GitHub.
- **Vercel / Netlify** — These are **external hosts** that connect to your repo. They can either (a) run their own build and deploy when you push, or (b) use a build you already did (e.g. from GitHub Actions). So CD is “when I push, this service deploys” — either by building themselves or by taking your CI-built artifact.

**In short:** we’re making “push to main” automatically update the live site, using either a second job in GitHub Actions (Pages) or a connected host (Vercel/Netlify). We do it so deployment is automatic and the live site stays in sync with `main`.

---

## 6b. Optional: Deployment options (how to add CD)

Once CI is stable, you can add **deployment** so every green build on `main` goes live. Choose one of the following.
...
3. Add env vars in Vercel dashboard if your app needs them.  
4. **Deploy only after CI passes (recommended):** By default Vercel deploys as soon as you push, before your GitHub Actions CI (lint, test, build) finishes. To make production update only after CI is green:
   - In your **CI workflow** (`.github/workflows/ci.yml`): add `permissions: contents: read; statuses: write` to the job, and add a final step that notifies Vercel using `vercel/repository-dispatch/actions/status@v1` with a check name (e.g. `ci-build-and-test`). See [CD-VERCEL-AFTER-CI.md](./CD-VERCEL-AFTER-CI.md) for the full flow.
   - In **Vercel**: Project → **Settings → Deployment Checks** (or under Deployment Protection). Add a check with the same name you used in the workflow (e.g. `ci-build-and-test`). Vercel will then wait for that check to pass before promoting the deployment to your production URL.
